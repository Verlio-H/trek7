...
CV ELSE (IV=1) WEB ESCAPE SUCCESSFUL!
      ...
      WRITE(1+L,1001)(INAME(IQ0,IT),IQ0=1,3)
...
CV WRITE TRAPPING MESSAGE
      WRITE(1+L,1003)(INAME(IQ0,IT),IQ0=1,3)
...
CV IF ACTUAL SPEED > AVAILABLE SPEED
      IF(WARP(IT).GT.TWARP(IT))WARP(IT)=TWARP(IT)
...
CV WRITE MESSAGE "CAUGHT IN A DEFLECTOR BEAM"
      WRITE(1+L,223)(INAME(IQ0,I7),IQ0=1,3)
...
222   IF(MIN.EQ.0)GO TO 224
      WRITE(1+L,225)(INAME(IQ0,I7),IQ0=1,3)
...
770   CALL HORTA(IENTR(IT),IENTC(IT),ITROW(IT),ITCOL(IT),Q,
     *RTZRO,ITVL1,RTZRO,IGNORE,RTZRO,itzro_array)
...
CV SET TERMINAL INDEX TO J*2+3
      N=J*2+3

CV WRITE COLLISON MESSAGE
      WRITE(1+N,235)(INAME(IQ0,IT),IQ0=1,3),(IENM2(IV,MIN),IV=1,4)
235   FORMAT(' THE ', 3A4, ' HAS COLLIDED WITH THE ',4A4/' WHICH HAS EXP
     *LODED')
...
CV WRITE RAMMING MESSAGE
      WRITE(1+N,168)(INAME(IQ0,IT),IQ0=1,3),(INAME(IQ0,MIN),IQ0=1,3)
...
CV CALCULATE RANDOM PLANET NUMBER I8
      ...
      WRITE(1+L,553)I8,LI2C(I7),LI2R(I7)
...
CV WRITE MESSAGE "ACQUISITION AND PROCESSING OF DILITHIUM CRYSTALS..."
      WRITE(1+L,551)LI2C(I7),LI2R(I7),J
...
5323  IF(MIN.GT.18)GO TO 5322
      ...
      N=IV*2+3
      WRITE(1+N,16)(INAME(IQ0,IT),IQ0=1,3),
...
5332  DO 226 N=1,4
      ...
      IVV=N*2+3
      WRITE(1+IVV,5333)(INAME(IQ0,IT),IQ0=1,3),IV,I8,I7,IV,
...
CV CALCULATE DAMAGE NUMBER IVV FROM DEFLECTOR STRENGTH AND DAMAGE FACTOR IV
      IVV=int(ALOG((101.-DFLCT(IT))*IV+10.)/0.700619195-1.8185)
...
CV
     ...
     *RTVL1,RTZRO,ITZRO,RTZRO,ITVL1,RTZRO,itzro_array)
...
CV WRITE "NACELLE HIT ..." MESSAGE
      WRITE(1+IV,204)(INAME(IQ0,IT),IQ0=1,3),J,(INAME(IQ0,MIN),IQ0=1,3)
...
CV WRITE "NACELLE ... HIT" MESSAGE
      WRITE(1+IV,208)(INAME(IQ0,IT),IQ0=1,3),J,(IENM2(IVV,MIN),IVV=1,4)
...
CV WRITE "NACELLE ... HIT ... STARBASE" MESSAGE
      WRITE(1+IV,212)(INAME(IQ0,IT),IQ0=1,3),J,(ISIDE(IQ0,MI),IQ0=1,3),
     *IBASE(MI)
...
CV WRITE "NACELLE ... HIT STAR AT ..." MESSAGE
      WRITE(1+IV,214)(INAME(IQ0,IT),IQ0=1,3),J,I8,I7
...
CV WRITE MESSAGE "NACELLE ... DESTROYED EAGLE ..."
      WRITE(1+IV,219)(INAME(IQ0,IT),IQ0=1,3),J,MIN
...
CV WRITE MESSAGE "NACELLE ... DESTROYED MINE AT ..."
      WRITE(1+IV,221)(INAME(IQ0,IT),IQ0=1,3),J,I8,I7
...
CV DECREASE NUMBER OF TORPEDOES TO 75%
      IPHOT(IT)=int(FLOAT(IPHOT(IT))*0.75)
...
CV ELSE: FAILED TO PIERCE THE WEB
      WRITE(1+L,1001)(IBOLT(J,JS),J=1,4)
...
CV WRITE MESSAGE
      WRITE(1+I8,105)(IBOLT(J,JS),J=1,4),(INAME(IQ0,IT),IQ0=1,3)
...
CV SEE WHAT THE TORPEDO HAS HIT (MIN)
     ...
     *ITZRO,RANG(IT),ANGLE(IT),J,AJUST,IITVL1,RTZRO,KPLOT)
...
CV WRITE "HIT ON THE <M-O SHIP>"
      WRITE(1+I8,108)(INAME(IQ0,IT),IQ0=1,3),(IBOLT(J,JS),J=1,4),
...
CV 4 < MIN <= 12
      ...
      I8=I7*2+3
      WRITE(1+I8,111)(INAME(IQ0,IT),IQ0=1,3),(IBOLT(J,JS),J=1,4),(
     *IENM2(J,MIN),J=1,4)
...
CV WRITE MESSAGE "<WEAPON TYPE> HAS SCORED A HIT ON A STAR"
      WRITE(1+L,526)(IBOLT(J,JS),J=1,4)
...
CV WRITE MESSAGE <M-O SHIP> HAS HIT <STARBASE OWNER PARTY> STARBASE <NUMBER>
      WRITE(1+I8,122)(INAME(IQ0,IT),IQ0=1,3),(IBOLT(J,JS),J=1,4),
...
CV WRITE MESSAGE "<M-O SHIP> <WEAPON TYPE> DESTROYED EAGLE <NUMBER>"
      WRITE(1+I8,115)(INAME(IQ0,IT),IQ0=1,3),(IBOLT(J,JS),J=1,4),IV
...
CV 14 < MIN <= 18 (???)
      ...
      WRITE(1+L,117)(IBOLT(J,JS),J=1,4),I8,I7
...
118   IF(I8.EQ.IENTC(IT).AND.I7.EQ.IENTR(IT))GO TO 557
      WRITE(1+L,81)(IBOLT(J,JS),J=1,4),I8,I7
81    FORMAT(1X,4A4,'FAILED TO HIT ANYTHING'/' NOW A MINE AT (',I2,',',
     *I2,')')
      MINES=MINES+1
      MAP(I7,I8)=IM(IT)
      GO TO 50
557   WRITE(1+L,806)(IBOLT(J,JS),J=1,4)
...
1010  N=int(DI(IENTR(IT),IENTC(IT),IROWA(IT),ICOLA(IT)))
...
CV IF ...: IVV=I*2+3
      IF(MUDD(J,IV1,I7,I8,N))IVV=I*2+3
...
801   IF(I.GT.4)GO TO 1250
      ...
      I8=I7*2+3
      WRITE(1+I8,127)(INAME(IQ0,IT),IQ0=1,3),(INAME(IQ0,I),IQ0=1,3)
...
CV IF M-O SHIP NOT IN GAME: NEXT M-O SHIP
      ...
      I8=I7*2+3
      WRITE(1+I8,130)(INAME(IQ0,IT),IQ0=1,3),(IENM2(J,I),J=1,4)
...
CV IF M-O SHIP NOT IN GAME: NEXT M-O SHIP
      ...
      I8=I7*2+3
      WRITE(1+I8,133)(INAME(IQ0,IT),IQ0=1,3),(ISIDE(IQ0,I),IQ0=1,3),
     *IBASE(I)
...
131   I=I-14
      ...
      I8=I7*2+3
      WRITE(1+I8,1350)(INAME(IQ0,IT),IQ0=1,3),I
...
1016  MIN=int(SQRT(FLOAT(I8**2+I7**2)))
...
CV WRITE MESSAGE
      IF(NUM.LT.0.AND.NDEAB(I).EQ.0)WRITE(1+L,213)(ISIDE(IQ0,I),IQ0=1,
     *3),IBASE(I)
...
CV ASK FOR ALLOWANCE TO BEAM OFF
      WRITE(1+I8,13)(INAME(IQ0,IT),IQ0=1,3),IV
...
CV WRITE MESSAGE "NOT ALLOWED"; RETURN
      WRITE(1+L,15)(INAME(IQ0,I),IQ0=1,3)
...
CV AND THAT SHIP IS BACK IN CONTROL
      WRITE(1+I8,52)(INAME(IQ0,IT),IQ0=1,3)
...
11    WRITE(1+I8,12)(INAME(IQ0,IT),IQ0=1,3)
...
140   I7=int((120.-(MA(IT,JTK)+1)*DEFLT)*FLOAT(NUM)/100./DI(IENTC(IT),
     *IENTR(IT),ICOL,IROW))
...
32    MIN=int(ISPRED*IHORDE/FLOAT(33-NODEK)+5)

      CALL RANDO(IV,0,MIN)
      IV=int(IV*A*ADED)
      IHORDE=IHORDE-IV
      IF(IHORDE.LE.0.OR.NODEK.GE.33)GO TO 35
39    WRITE(1+I8,37)IHORDE,I7
...
44    WRITE(1+I8,52)
52    FORMAT(' ENEMY-HELD AREAS SEALED OFF -  ENEMY NOW RESORTING TO BLA
     *STING THROUGH BULKHEADS')
...
63    DO 75 IV=1,4
      ...
      I8=IV*2+3
      WRITE(1+I8,64)(INAME(IQ0,IT),IQ0=1,3)
...
CV WRITE BOMB MESSAGE
      ...
      WRITE(1+MIN,68)(INAME(IQ0,I),IQ0=1,3)
...
72    WRITE(1+I8,74)(INAME(IQ0,IT),IQ0=1,3),IFROM,DFLCT(IT),PHASR(IT),
...
206   CALL HORTA(IPUR,IPUC,ITZRO,ITZRO,RAD,BERNG,ITZRO,
     *RTZRO,IGNORE,RTZRO,itzro_array)
...
CV MINE HIT VESSEL
CV ================
      WRITE(1+I81,272)(INAME(IQ0,MIN),IQ0=1,3)
...
CV ===================
271   WRITE(1+I81,273)IVV,(INAME(IQ0,MIN),IQ0=1,3)
...
CV NEITHER M-O NOR C-O SHIP: 205
      ...
      WRITE(1+I81,207)(IENM2(I,MIN),I=1,4)
...
CV ===================
210   WRITE(1+I81,211)IVV,(IENM2(I,MIN),I=1,4)
...
276   MIN=I71
      ...
      WRITE(1+I81,279)(ISIDE(IQ0,MIN),IQ0=1,3),IBASE(MIN)
...
CV ===================
278   WRITE(1+I81,280)IVV,(ISIDE(IQ0,MIN),IQ0=1,3),IBASE(MIN)
...
243   I71=I7
     ...
     *RTZRO,ITZRO,RTZRO,ITVL1,RTZRO,itzro_array)
