...
CV COMMAND CANCEL
      ...
      DIMENSION IGOK(5)
      character(4):: igol, igok, iblk, map, ienm1, iee, igle, im, iii
      character(4):: istar
      character(4):: na
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
...
7100  WRITE(1+L,73)IPHOT(IT),(IBOLT(I,IT),I=1,4)
73    FORMAT(' YOU HAVE',I3,1X,4A4/' ENTER ANGLE: ')
      READ(L,*,ERR=130)ANGLE(IT)
...
CV DEFLECTOR TARGET SETTING
      ...
      character(4):: map, iblk, ienm1, iee, igle, im, iii, istar
      character(4):: na, igol
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
     ...
     *IPUSHR(4),IPUSHC(4)
      ...
      PUSH(IT)=A
      IPUSHC(IT)=int(B/100.)
      IPUSHR(IT)=int(B-IPUSHC(IT)*100.)
      GO TO 62
61    WRITE(1+L,21)DISTP
...
36    FORMAT(' IF THE TARGET IS A SHIP-'/' THE AMOUNT OF ENERGY (E) ENTE
     ...
     * 2**(-E/5)'/' EG. 5 WILL CUT THE SHIP''S SPEED IN HALF, '/' 10 WIL
     *L SLOW IT TO 1/4 SPEED, 15 WILL SLOW IT TO 1/8 SPEED'//' IF THE TA
...
CV
     ...
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      ...
      character(4):: na, nb, len2, nc, map, iblk, iii, im, ienm1, istar
      character(4):: igle, iee
...
CV LOOP OVER COLS (J) FOR IV TO IVV
      ...
      LEN(J-IV+1)=J-(J/10)*10
552   continue
      WRITE(1+L,5175)(LEN(J),J=1,IVV-IV+1)
5175  FORMAT(3X,61I1)
...
CV WRITE LEFT ROW AXIS, LEN2(I) [MAP ROW SYBOLS], RIGHT ROW AXIS TENNER,ONER
      WRITE(1+L,5172)IV1,(LEN2(I),I=1,JTK),NB,NA
5173  continue
...
CV PRINT EAGLE POSITION
      WRITE(1+L,303)I,IGLEC(I),IGLER(I)
303   FORMAT('+',I2,' - (',I2,',',I2,')  ', $)
...
CV GET INFO ("IN" COMMAND)
     ...
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      ...
      COMMON /V/IWEB(2),IWEBZ,INVIS(4)
      WRITE(1+L,587)(INAME(IQ0,IT),IQ0=1,3),DFLCT(IT),IENTC(IT),
     *IENTR(IT),NDEAD(IT)
...
CV WRITE SHIP, AVAILABLE DEFLECTOR STRENGTH AND POSITION
      WRITE(1+L,771)(INAME(IQ0,N),IQ0=1,3),DFLCT(N),IENTC(N),IENTR(N)
...
CV IF NOT IN GAME: GOTO 772 (NO WRITE)
      ...
      WRITE(1+L,773)(ISIDE(IQ0,N),IQ0=1,3),IBASE(N),DFLCB(N),IBASC(N),
     *IBASR(N)
...
761   WRITE(1+L,755)(IENM2(I,N),I=1,4),DFLCK(N),IKLNC(N),IKLNR(N)
755   FORMAT(1X,4A4,7X,F5.1,4X,'(',I2,',',I2,')')
      GO TO 760

762   IF(INVIS(IT).EQ.1)GO TO 761
      WRITE(1+L,757)(IENM2(I,2),I=1,4)
757   FORMAT(1X,4A4,7X,'UNKNOWN  UNKNOWN')
      GO TO 760

763   WRITE(1+L,756)(IENM2(I,N),I=1,4),IKLNC(N),IKLNR(N)
...
764   IF(IARMZ(4).EQ.1.AND.IMAGZ.GT.0)GO TO 91
      ...
      WRITE(1+L,755)(IENM2(I,6),I=1,4),DFLCK(6),IKLNC(6),IKLNR(6)
      GO TO 760

91    WRITE(1+L,757)(IENM2(I,6),I=1,4)
760   CONTINUE
      RETURN
      END
...
800   FORMAT('  IF YOU WISH TO ENTER THE COORDINATES (60,60) OR (1, 1),
     *'/' TYPE 6060 OR 0101.')
      WRITE(1+L,801)
801   FORMAT( ' THE RANGE OF A PHOTON TORPEDO OR DISRUPTOR BOLT IS 10 UN
     *ITS'/' NOTE ALSO THAT THERE IS A MAXIMUM AMOUNT OF DAMAGE THAT CAN
     ...
     *OUT 500 UNITS PER SHOT')
      WRITE(1+L,33)
33    FORMAT(' IF WE ENTER AN ION STORM WE WILL RECEIVE DAMAGE'/' AND OU
     *R SPEED WILL BE CUT IN HALF')
      WRITE(1+L,38)
38    FORMAT(' ENEMY STRATEGY AND STRENGTH-')
      WRITE(1+L,37)
35    FORMAT(' KLINGONS ALWAYS ATTACK'/' ROMULANS RETREAT WHEN YOUR SHIE
     *LDS EXCEED THEIRS BY 20,'/' AND DO NOT TRAVEL ON A DIRECT COURSE T
     *O OR FROM YOU WHEN ATTACKING'/' OR RETREATING'/' THOLIANS ALWAYS A
     *TTACK, BUT HALT WHEN THEY ARE 8 UNITS AWAY AND USE THEIR WEB',
...
CV
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      ...
      character(4):: map, iblk, ienm1, na, iee, igle, im, iii, istar
      character(4):: mes
...
CV INFORM LOOPED M-O SHIP ABOUT DISTRESS SIGNAL
      WRITE(1+I7,9)(INAME(IQ0,IT),IQ0=1,3)
...
CV ASK ADDRESSED SENDER TO SEND
      WRITE(1+I8,28)(INAME(IQ0,IT),IQ0=1,3)
...
CV WRITE MESSAGE TO ADDRESSED M-O SHIP
      WRITE(1+I8,34)(INAME(IQ0,IT),IQ0=1,3),MES
...
33    WRITE(1+L,34)(INAME(IQ0,I),IQ0=1,3),MES
...
370   N=IBPSC(JTK)*2+3
      ...
      WRITE(1+N,380)(INAME(IQ0,I8),IQ0=1,3),(INAME(IQ0,ISTAT),IQ0=1,3),
     *MES
...
CV TRANSFER COMMAND
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      ...
      character(4) :: na, ienm1, iee
      character(4):: igo

625   WRITE(1+L,600)DFLCT(IT),PHASR(IT),TWARP(IT),IPHOT(IT)
...
650   FORMAT(' TYPE IN 2 LETTERS (A2) - 1ST LETTER IS THE SYSTEM YOU WIS
     *SH TO TRANSFER ENERGY FROM'/' AND THE 2ND LETTER IS WHERE YOU WISH
     * TO TRANSFER IT TO'/' (WARP ENGINES (W) , PHASER BANKS (P), TORPED
     *O /DISRUPTOR BANKS (T) DEFLECTOR SHIELDS(D))'
     */' (PW,WP,WD,DW,PD,DP,TD'/)
      WRITE(1+L,39)
...
6500  FORMAT(' THE ''DD'' COMMAND TRANSFERS DEFLECTOR SHIELD ENERGY BETW
     ...
     *CTORS. '/' TYPE A NEGATIVE NUMBER TO TAKE THAT AMOUNT OF ENERGY FR
     *OM'/' THE OTHER SHIP AND PUT IT IN YOUR DEFLECTORS (YOU CAN ONLY T
     *AKE'/' ENERGY FROM A BOARDED SHIP). '/' YOU''LL ALSO BE ASKED TO E
     *NTER THE LETTER OF THE SHIP YOU WISH'/' TO TRANSFER ENERGY WITH - 
     *TYPE ''K'' FOR KLINGON, ''R'' FOR ROMULAN'/' ''Z'' FOR KZINTI, ETC
     *.'//' IF YOU TYPE ''**'', YOU CAN TRANSFER ENERGY, PERSONNEL AND T
     *ORPEDOES WITH STARBASE')
      GO TO 632
...
7     WRITE(1+L,5)NDEAB(I),NDEAD(IT)
...
10    WRITE(1+L,8)DFLCB(I),DFLCT(IT)
...
12    WRITE(1+L,11)IPHOB(I),IPHOT(IT)
...
CV REQUEST FOR A NEW COMPUTER-CONTROLLED ENEMY
     ...
     *IBASR(2),IBASC(2),LI2(4),LL2R(5),LI2C(5),IGO(4),MINES
      ...
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
      COMMON /U/LAUNCH,NUMOUT,NUME(2)
      character(4):: na, ienm1, map, iblk, iii
...
710   WRITE(1+L,711)(IENM2(IV,I),IV=1,4)
...
58    WRITE(1+L,21)DISTP
21    FORMAT(' RANGE OF BEAM IS', F4.0, ' UNITS'/' ENTER TARGET COORDINA
     *TES:  ', $)
...
CV ELSE (WARP ENERGY < 0): WRITE HELP
      WRITE(1+L,41)
41    FORMAT(/' IF THE TARGET IS A SHIP-'/' THE AMOUNT OF ENERGY (E) ENT
     *ERED WILL SLOW THE RETREATING ENEMY'/' SHIP''S SPEED BY A FACTOR O
     *F 2**(-E/0. 5) '/' EG. 0.5 WILL CUT THE SHIP''S SPEED IN HALF, '/
     *'1.0 WILL SLOW IT TO 1/4 SPEED, 1.5 WILL SLOW IT TO 1/8 SPEED'
     *//'IF THE TARGET IS AN EAGLE OR MINE-'/' 0.1 UNITS OF WARP ENERGY
     *ARE NEEDED FOR EACH UNIT THIS OBJECT IS PULL-ED IN'//' IF THE TARG
     *ET IS EMPTY SPACE OR AN ION STORM- NOTHING WILL HAPPEN'//' IF THE
     *TARGET IS A STAR, DOOMSDAY MACHINE OR MOONBASE ALPHA-'/
     *'NOTHING WILL HAPPEN AS THE TARGET IS TOO MASSIVE FOR THE TRACTORS
     * TO HANDLE'/)
...
22    FORMAT( ' PROB. OF US LOSING STEERING CONTROL AND VEERING OFF COUR
     ...
     *ENGINES WILL BE SHUT DOWN. '/' TO MOVE THEREAFTER YOU WILL HAVE TO
     * BOOST WARP ENERGY AND'/' WAIT A TURN TO RE-ENERGIZE THE ENGINES')
      GO TO 2
...
30    FORMAT(2I2,1X,2I2)
      ...
      WRITE(1+L,31)DISTP,AJUST
31    FORMAT(' DISTANCE=',F0.5/' PROBABILITY OF A PHASER HIT AT THIS DI
     *STANCE=',F8.4,'%')
      GO TO 2
...
CV EVERYTHING OK, MESSAGE "PHASERS LOCKED ON TARGET"
      WRITE(1+L,504)ICOLA(IT),IROWA(IT)
...
CV ASK FOR STARBASE NUMBER, READ
      WRITE(1+L,3)
C3     FORMAT(' ENTER STARBASE NUMBER: '$)
...
CV WRITE "STARBASE CEASED FIRE" MESSAGE
      WRITE(1+IV,9)(ISIDE(IQ0,I),IQ=1,3),I7
...
8     WRITE(1+IV,10)(ISIDE(IQ0,I),IQ=1,3),I7
...
CV
      WRITE(1+IV,109)(IENM2(N,I),N=1,4)
109   FORMAT(1X,4A4,' RETURNING TO NEAREST ALLIED VESSEL')
      GO TO 107

108   WRITE(1+IV,110)(IENM2(N,I),N=1,4)
