...
C TYPE-EXACT CHECK 04/26/2000 BY: D.G.
      ...
      character(4) :: CMAND(26)
      LOGICAL ITAKA(6)
      character(4):: no, igol, iblk
      character(4) :: maddr(7)
...
9988  FORMAT(/' DONALD ECCLESTONE (C)1979 AND DAN GAHLINGER (C)1993
     ...
     *   T R E K 7'///)
      WRITE(1+5,*)' TYPE IN A RANDOM NUMBER BETWEEN 1-32767'
      READ(5,*)ITOM
...
CV GET (NEXT) COMMAND
234   WRITE(1+L,10)(INAME(IQ0,IT),IQ0=1,3)
...
CV IF I8=0: NO DAMAGE TO SHIP
      IF(I8.EQ.0)WRITE(1+L,3612)
C3612  FORMAT(' NO DAMAGE TO SHIP')
...
CV WRITE NUMBER OF SURVIVORS OF M-O SHIP (NDEAD(I))
      IF(IBPSC(I).EQ.IBPSC(IT))WRITE(1+L,353)NDEAD(I),
     *(INAME(IQ0,I),IQ0=1,3)
...
CV WRITE NUMBER OF SURVIVORS OF STARBASE NDEAB(I)
      IF(IBPSS(I).EQ.IS)WRITE(1+L,146)NDEAB(I),IBASE(I)
...
CV ELSE: WRITE NUMBER OF CREW ON BOARDED VESSEL
      WRITE(1+L,356)IBPOB(I),(IENM2(N,I),N=1,4)
...
CV WRITE SURRENDER WISH MESSAGE TO ALL OTHER M-O SHIPS
      WRITE(1+I7,171)(INAME(IQ0,IT),IQ0=1,3)
...
5190  WRITE(1+L,910)(INAME(IQ0,IT),IQ0=1,3)
910   FORMAT(/' THE STARSHIP ',3A4,' IS NOW RAISING THE WHITE FLAG')

CV WRITE DEMOTION MESSAGE
      WRITE(1+L,91)IBASE((IBPSC(IT)+1)/2),(INAME(IQ0,IT),IQ0=1,3)
91    FORMAT(/' STARBASE',I3,' CALLING THE ',3A4,/' DUE TO YOUR COWARDLY
     * SURRENDER, WE HAVE LOST FACE.'/' FOR THIS ACT YOU ARE DEMOTED TO
...
C90    FORMAT(//' ',15('-'),'  KEEP ON TREKKIN''   ',15('-'),//' IF ANY E
C     *RRORS ARE DETECTED, PLEASE SEND ME A COPY OF THE'/' ERRONEOUS PART
C     * OF THE GAME. ALSO, IF YOU HAVE ANY COMMENTS, '/' COMPLAINTS, COMP
C     *LIMENTS, QUESTIONS OR SUGGESTIONS ABOUT THE'/' PROGRAM, I AM ANXIO
C     *S TO HEAR THEM.'/////', CONTACT-'//'   DONALD ECCLESTONE,'/,5X,
C     *'C/O DAN GAHLINGER VIA,'/' DAN@VRX.NET -',5X)
C      STOP

CV NEW TRAILER

      WRITE(1+LAST,90)(MADDR(I),I=1,7)
...
335   FORMAT(' ENTER A STRING OF CHARACTERS INDICATING THE SEQUENCE IN W
     ...
     *' ''P'' TO FIRE PHASERS'/
     *' ''T'' TO FIRE TORPS/DISRUPTORS'/' ''B'' TO INITIATE A BOARDING P
     *ARTY'/
     *' ''Y'' TO ENGAGE A TRACTOR BEAM'/' AND/OR ''D'' TO ENGAGE A DEFLE
     *CTOR BEAM. '/' EG. IF YOU WISH TO FIRE A TORPEDO THEN ENGAGE A TRA
     *CTOR BEAM THEN INITIATE A'/' BOARDING PARTY THEN MOVE THEN FIRE PH
     *ASERS THEN ENGAGE A DEFLECTOR BEAM,'/' TYPE ''TYBMPD''.'/
     *' NOTE THAT YOU DON''T HAVE TO USE ALL THE CHARACTERS-'/
     *' COMMANDS SUCH AS ''M'' OR ''PT'' OR ''BY'' ARE POSSIBLE.')
      WRITE(1+L,336)
...
337   FORMAT(' OTHER FORMS--'/' EG. B(M)(PT) -IF THE BOARD IS SUCCESSFUL
     *, WE MOVE ELSE WE FIRE PHASERS AND TORPEDOES. '/' THUS THE BACKUS-
     *NAUR FORM OF THE GO-LINE IS-'/
     *' <GO-LINE>::=<GO-CHAR>  / <GO-CHAR>(<GO-LINE>)(<GO-LINE>) / <GO-C
     *HAR>(<GO-LINE>) / NIL'/'<GO-CHAR>::=M/P/T/B/Y/D'/' CAUTION -THE GO
     *-LINE SHOULD BE 80 CHARACTERS OR LESS IN LENGTH AND SHOULD NOT NES
     *T MORE THAN 6 LEVELS.')
...
C
      ...
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      character(4):: ienm1, iblk, istar, iii, igle, iee, im
      character(4) :: ienm2, iname, iside
...
CV WRITE MESSAGE "OUR SIDE HAS DEFEATED THE ENEMY"
      WRITE(1+J,767)(ISIDE(IQ0,JTK),IQ0=1,3),IBASE(JTK),
     *(INAME(IQ0,I),IQ0=1,3)
...
C     SECS=SECNDS(0.0)*100.0
C     ISECS=INT(SECS)

CV CALCULATE IMPROVED ITOM
C     ITOM=INT(MOD((ITOM+ISECS),32767))
...
C      RT=RAN(ISHRT,ISHIT)
      ...
      ITOM=IFIX(RT*10000.)
      TEMP=FLOAT(MOD(ITOM,100))/100.
      IX=int((IZ-IY+1)*TEMP+IY)
...
CV
     ...
     *,DIST,KPLOT)
C      INTEGER KPLOT(10),INTER(19)
      integer :: kplot(10)
      character(4):: inter(19)
      LOGICAL CYRANO
...
CV            IS ALREADY DECLARED AS SUBROUTINE PARAMETER!
     ...
     *'=','1','2','3','4','I'/
      character(4):: na, map, iblk, ienm1, iee, iii, istar, igol
C      IF(IO.EQ.1)TYPE 1970,N,ISTR,ISTC,ITAR,ITAC,RAD,BERNG,AJUST,IGNORE
      if (io == 1) print 1970, N, ISTR, ISTC, ITAR, ITAC, RAD, BERNG,
     *ADJUST, IGNORE
...
C      IF(IO.EQ.1)TYPE 1999,ITAC,ITAR,ISTC,ISTR,X
      if (io == 1) print 1999, ITAC, ITAR, ISTC, ISTR, X
...
CV RESET I7 TO ...
      I7=int(ISTR+SIGN(FLOAT(J),SINA))

CV RESET I8 TO ...
      I8=int(ISTC+SIGN(FLOAT(M),COSA))
