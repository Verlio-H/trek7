...
CV WRITE MESSAGE
CV "THE <M-O SHIP NAME> HAS BEEN ENGULFED BY THE EXPLOSION ..."
      WRITE(1+IV,772)(INAME(IQ0,KOENIG),IQ0=1,3)
...
CV WRITE MESSAGE "... EXPLOSION FROM DOOMSDAY"
      WRITE(1+I7,59)(ISIDE(IQ0,N),IQ0=1,3),IBASE(N)
...
CV "... AN EXPLOSION HAS PULVERIZED THE <C-O SHIP> ..."
      WRITE(1+J,102)(IENM2(MIN,N),MIN=1,4)
102   FORMAT(/' SENSORS REPORT THAT AN EXPLOSION HAS PULVERIZED THE ',
     *4A4/'    INTO POWDERED DUST')
...
CV WRITE MESSAGE "... DESTRUCTION OF <PARTY> STARBASE <NUMBER>"
      WRITE(1+N,157)(ISIDE(IQ0,I),IQ0=1,3),IBASE(I)
...
CV SET "NO CREW" FLAG
      ...
      N=I7*2+3
      WRITE(1+N,159)(ISIDE(IQ0,I),IQ0=1,3),IBASE(I)
...
CV SET J TO PARTY INDEX
      ...
      WRITE(1+IVV,238)IBASE(J),(INAME(IQ0,I),IQ0=1,3)
...
CV IF LOOPED SHIP IS IN GAME: WRITE DESTRUCTION MESSAGE
      IF(ICHOE(I7).NE.0)WRITE(1+IV,136)(INAME(IQ0,I),IQ0=1,3)
...
C
C      OPEN(UNIT=1,TYPE='OLD',READONLY,
C     *ORGANIZATION='SEQUENTIAL',NAME='KIRK.DAT')
      open(unit=1, status='old', file='kirk.dat')
C      OPEN(UNIT=2,TYPE='OLD',READONLY,
C     *ORGANIZATION='SEQUENTIAL',NAME='KOLOTH.DAT')
      open(unit=2, status='old', file='koloth.dat')
      DO 1 I=1,33
C      READ(1'I,10)(IDEX(1,I,J),J=1,20)
C 1     READ(2'I,10)(IDEX(2,I,J),J=1,20)
      READ(1,10)(IDEX(1,I,J),J=1,20)
      READ(2,10)(IDEX(2,I,J),J=1,20)
1     continue
10    FORMAT(20A4)
...
CV SETTING TERMINALS, CHOOSING OF ENEMIES AND NUMBER OF STARS, AND ???
      ...
      SUBROUTINE FIZBIN
C      INTEGER LAP(3600),LA(132),LK(56),IGOM(8)
      integer :: la(132), lk(56)
      character(4):: lap(3600), igom(8)
      EQUIVALENCE(LAP,MAP),(LA,MA),(LK,K)
C***
C      INTEGER ITOM2,IDEV(2)
      integer :: itom2
      character(4) :: idev(2)
      INTEGER*2 ICNT
C USED BY VAX VERSION
     ...
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      ...
      COMMON /W/IDEX(2,33,20)
      COMMON /TOM/ITOM
      character(4):: iee, ienm1, map, iii, istar, na, iblk
...
104   FORMAT(' ENTER THE MANUALLY-OPERATED SHIPS YOU WISH BY ENTERING A
     *STRING CONTAINING-'/' ''E'' FOR ENTERPRISE, ''P'' FOR POTEMPKIN (F
     *EDERATION),'/' ''H'' FOR HAVOC, ''C'' FOR CARNAGE (KLINGON)')
CV CHANGED UNIT FROM L TO 5
...
109   FORMAT(' IT IS TIME TO ASSIGN TERMINALS TO TREK7. TO DO THIS YOU N
     *EED TO KNOW'/' THE DEVICE NAME OF THE TERMINALS THAT YOU WILL BE U
     *SING.'/' IF YOU DO NOT KNOW THIS THEN STOP THE GAME AND FIND OUT.'
     */' TO DO THIS TYPE "SHOW TERM", THE FIRST PIECE OF INFORMATION IS 
     *THE NAME OF OUR TERMINAL'/
...
CV CHANGED UNIT FROM L TO 5
921   WRITE(1+5,9211)
9211  FORMAT(' IF YOU WANT THE ENTERPRISE TO PLAY OFF OF THIS TERMINAL T
     *HEN TYPE A BLANK.'/' DO NOT TYPE IN THE NAME OF THIS TERMINAL, THI
     *S CAUSES THE GAME TO DIE.')

CV CHANGED UNIT FROM L TO 5
C      READ(5,9212)(IDEV(J),J=1,2)
C9212  FORMAT(2(A4))
      read(5,'(A)') file_name

C      IF(IDEV(1).EQ.' ')GOTO 926
      if (file_name == '') goto 926
      open(6,file=file_name)
      write(6,'(A, $)') 'input file: '
      read(5,'(A)') file_name
      open(5,file=file_name)
...
CV WRITE NO-CHOOSER MESSAGE
      WRITE(1+I8,114)(INAME(IQ0,I7),IQ0=1,3)
...
CV IF M-O SHIP IS IN GAME: WRITE SHAKEDOWN MESSAGE
      IF(ICHOE(J).EQ.1)WRITE(1+I,6100)
...
C this seems suspicious, the gotos from above still add a star but don't increment counter
...
CV SKIP PREPARE FOR KZINTI
      IF(I.NE.6.OR.ICHOS(6).EQ.0)GO TO 750
...
C seems to be completely unused??
C99    M=I(1)/2 + I(2)/128 + D/2 + A/128 + 2653589
99    continue
...
C      DECODE(2,4,D)M
C very suspicious reads
      read(D, 4) M
4     FORMAT(I2)

      IF(M.LT.10.OR.M.GT.16)RETURN

C      DECODE(9,1,I)D,A,Y
C very suspicious reads
      read(I, 1) D, A, Y
...
CV IF M-O SHIP MIN NOT IN GAME: NEXT M-O SHIP
      ...
      I8=MIN*2+3
      WRITE(1+I8,2)(ISIDE(IQ0,J),IQ0=1,3)
...
CV IF M-O SHIP = WISHING M-O SHIP: NEXT M-O SHIP
      ...
      I8=IV*2+3
      WRITE(1+I8,6)(ISIDE(IQ0,J),IQ0=1,3),(INAME(IQ0,MIN),IQ0=1,3)
...
54    WRITE(1+L,55)
55    FORMAT(' ENTER NUMBER OF FALSE IMAGES TO BE GENERATED - AT MOST 15
     *')
      N=10
      READ(L,9,ERR=73)DISTP
...
51    WRITE(1+L,52)
52    FORMAT(' ENTER RANGE AT WHICH ENTERPRISE CAN DETECT THE KZINTI SHI
     *P'/' THROUGH THE CLOAKING DEVICE')
      N=12
      READ(L,9,ERR=73)DISTP
